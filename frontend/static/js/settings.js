(function () {
  ...
  const EPS = 1e-12;
  const LS_DC   = "aidarts_dc_enabled";
  const LS_CAMS = "aidarts_cam_assign";
  const LS_CAL  = "aidarts_calib";
  const LS_DET  = "aidarts_detected";
  ...
  /* ══════════════════════════════════════════════════════════
     Linear algebra — homography from 4-point pairs
     ══════════════════════════════════════════════════════════ */
  ...
  let detected  = lsGet(LS_DET, []);
  let assign    = lsGet(LS_CAMS, { 1: "", 2: "", 3: "" });
  let calibSave = lsGet(LS_CAL, {});   // slot → anchors (image-norm coords)
  ...
  const modal = {
    open: false,
    slot: null,
    anchors: null,   // { nw:[x,y], … } in image-normalised coords
    dragging: "",
    pointerId: null,
    handleEl: null,
  };
  ...
  toggleDC.checked = localStorage.getItem(LS_DC) !== "false";
  syncDC();
  toggleDC.addEventListener("change", () => { localStorage.setItem(LS_DC, toggleDC.checked); syncDC(); });
  ...
  $("#btn-folder").addEventListener("click", async () => {
    ...
    try { const h = await window.showDirectoryPicker(); folderName.textContent = h.name; folderName.classList.add("is-set"); } catch {}
  });
  ...
  selects.forEach((sel, i) => {
    sel.addEventListener("change", () => { assign[i+1] = sel.value; lsSet(LS_CAMS, assign); if (sel.value) loadPreview(i+1); else clearPreview(i+1); });
  });
  ...
  // Load previews sequentially — USB cameras on a hub fail if opened concurrently
  (async () => {
  ...
  /* ══════════════════════════════════════════════════════════
     ...
     Port of AI-Dart-Scorer manual calibration:
     - Image is fixed, wire SVG overlay is CSS-transformed.
     - 4 handles are dragged to where dartboard corners appear
       in the image.
     - A homography maps overlay-normalised coords → image-
       normalised coords. Its inverse is converted to a CSS
       matrix3d and applied to the wire SVG so it deforms to
       match the dartboard in the image.
     ══════════════════════════════════════════════════════════ */
  ...
  /* ── Build overlay→image homography from anchors ── */

  function buildOverlayH(anchors) {
    const src = HANDLE_KEYS.map(k => WIRE_SRC[k]);
    const dst = HANDLE_KEYS.map(k => anchors[k]);
    return h4pt(src, dst);
  }
  ...
  function renderWire() {
    ...
    // Position the wire layer over the overlay region
    ...
    // Compute homography: overlay-norm → image-norm
    const H = buildOverlayH(modal.anchors);
    if (H) {
      // We need overlay-norm → layer-pixel transform for the CSS
      const imgToLayer = [
    ...
    // Position handles in layer-normalised coords
  ...
  function closeModal() {
    endDrag(null, true);
    if (resetAbort) { resetAbort.abort(); resetAbort = null; }
    modal.open = false;
    modal.slot = null;
    modal.anchors = null;
  ...
  $("#btn-reset").addEventListener("click", () => {
    ...
    // Clear stored homography and anchors
    delete storedH[slot];
    lsSet(LS_H, storedH);
    delete calibSave[slot];
    lsSet(LS_CAL, calibSave);

    // Re-fetch original snapshot from camera and update modal
      ...
      fetch(`/api/cameras/${idx}/snapshot`)
        ...
        .then(blob => {
          ...
          loadImageFromUrl(url, img => {
            ...
            setPreviewEverywhere(slot, url, img);
            // Update modal with the fresh original image
  ...
  function applyPointer(e, L) {
    ...
    // Validate with the new point before allocating a clone
  ...
  /* ── Apply homography — warp image pixels, reset handles ── */
  ...
  function applyWarp() {
    ...
    // Skip if handles haven't moved
    let moved = false;
    for (const k of HANDLE_KEYS) {
      if (Math.abs(modal.anchors[k][0] - defaults[k][0]) > 0.002 ||
          Math.abs(modal.anchors[k][1] - defaults[k][1]) > 0.002) { moved = true; break; }
    }
    if (!moved) return;

    // Incremental H: maps default positions → current handle positions
    const dst = HANDLE_KEYS.map(k => defaults[k]);
    const src = HANDLE_KEYS.map(k => modal.anchors[k]);
